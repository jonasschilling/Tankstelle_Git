...
//erzeugt einen String fuer die Ausgabe eines Belegs.
...
//schreibt einen Beleg in eine Textdatei.
	public void writeReceipt() {
		...
		try {
			...
			if (numWodka > 0) {
				...
				bw.write("Wodka Jelzin - " + numWodka + " " + type + " " + price / numWodka + " "
						+ salesModel.getEurosign() + "/" + current.getType() + " - " + price + " "
						+ salesModel.getEurosign() + "\n");
			}
			if (numFilip > 0) {
				...
				bw.write("Filip Maurice - " + numFilip + " " + type + " " + price / numFilip + " "
						+ salesModel.getEurosign() + "/" + current.getType() + " - " + price + " "
						+ salesModel.getEurosign() + "\n");
			}
			if (numJupiter > 0) {
				...
				float price = (float) Math.round(numJupiter * Float.valueOf(salesModel.readPrice("Jupiter")) * 100)
						/ 100;
				bw.write("Jupiter Riegel - " + numJupiter + " " + current.getType() + " " + price / numJupiter + " "
						+ salesModel.getEurosign() + "/" + current.getType() + " - " + price + " "
						+ salesModel.getEurosign() + "\n");
			}
			if (numBull > 0) {
				...
				bw.write("Sitting Bull - " + numBull + " " + type + " " + price / numBull + " "
						+ salesModel.getEurosign() + "/" + current.getType() + " - " + price + " "
						+ salesModel.getEurosign() + "\n");
			}
			if (numPizza > 0) {
				...
				bw.write("Tk-Pizza Deluxe - " + numPizza + " " + current.getType() + " " + price / numPizza + " "
						+ salesModel.getEurosign() + "/" + current.getType() + " - " + price + " "
						+ salesModel.getEurosign() + "\n");
			}
...
//schreibt die Anzahl der Belege in eine Textdatei. (Wird gebraucht, um die Files beim Start wieder richtig auszulesen). 
		...
		try {
			...
			for (int i = 0; i < FinancesController.getReceipts().size(); i++) {
				...
				bw.write(FinancesController.getReceipts().get(i).getDate() + ";"
						...
						+ FinancesController.getReceipts().get(i).getPath());
			}
...
//Liest die Anzahl der Belege aus, sowie fügt die Belege wieder in die Applikation ein.
...
//erhoeht die Anzahl der Belege um 1.
